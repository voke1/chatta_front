...
export default class Convo extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            empty: {
                identity: "empty",
                prompt: "Sorry I don't understand what you said. Would you like to know about the following?",
                response: {
                    buttons: [
                        {
                            "key": "final_expenses",
    ...
    renderChatButtons = (buttons) => {
        return buttons.map(button => {
            return <button key={this.setUniqueKey(button.key, 'b')} type="button" className="ith_chat-button" onClick={() => this.updateConverstion(button.key, button.val)}>{button.val}</button>
        })
    }
    ...
    renderConversation = () => {
        ...
        return this.state.responses.map((convo, index) => {
            return (<li key={this.setUniqueKey(convo.id)} >
                {(!convo.selection) ?
                    (
                        <React.Fragment>
                            <div className="bot-div">
                                <div className="message-data">
                                    <span className="message-data-name"><i className="fa fa-circle me"></i> chatta</span>
                                    <span className="message-data-time">{this.state.times[index] || "Now"}</span>
                                </div>
    ...
    isThinking = () => {
        return (this.state.thinking) ? (<img src={thinker} className="thinker loader" />) : null;
    }
    ...
    searchKeywordsFromUserInput = (input) => {
        ...
        const treeNodes = conversationTree.map(node => {
            return (node.identity) ? (node.identity.toLowerCase().split('_').join(' ')) : '';
        })
        const userInputArray = userInput.split(' ');
        for (let i = 0; i < userInputArray.length; i++) {
            const keywordIndex = treeNodes.indexOf(this.checkMultiWordSearch(userInputArray, i));
            if (keywordIndex > -1) {
    ...
    checkMultiWordSearch = (userInputArray, i) => {
        return (userInputArray.length > 0) ? userInputArray.join(' ') : userInputArray[i];
    }

...
export default class AnalyticsHelper {
  ...
  findVisitsInRangeAndLimit = async (dates, botId, limit) => {
    ...
    const data = await apiService.get(
      "visitors",
      `${new Date(dates[0])}*${new Date(dates[1])}/${botId}/range/${limit}`
    );

    data.forEach(visit => visitsInRange.push(visit.visitors));
    console.log("bot id", visitsInRange);

    return visitsInRange;
  };
  findAllVisitsInRange = async (dates, botId, type) => {
    const visitsInRange = [];
    const param =
      type === "single"
        ? `${new Date(dates[0])}*${new Date(dates[1])}/${botId}/single/null`
        : `${new Date(dates[0])}*${new Date(dates[1])}/${botId}/range/null`;
    const dataForPaginate = await apiService.get("visitors", param);
  ...
  findAllVisitsAndLimit = async (botId, limit) => {
    ...
    const visits = await axios.get(
      `${BASE_URL}/visitors/all?limit=${parseInt(limit, 10) ||
        this.state.recordLimit}&&botId=${botId.toString()}`
    );
    console.log("haaaaa", visits);
    visits.data.forEach(visit => allVisits.push(visit.visitors));
    return allVisits;
  };
  fetchVisits = async (dates, limit, type, botID) => {
    ...
    try {
      ...
      if (dates) {
        ...
        const allVisitsInRange = await this.findVisitsInRangeAndLimit(
          dates,
          botID,
          10
        );
        console.log("visits in range", data);
  ...
  getTodayData = async (botId, newTime, type) => {
    ...
    let allVisits = [];
    try {
      allVisits = await this.findAllVisitsInRange(dates, botId, type);
  ...
  collectVisits = async botId => {
    ...
    try {
      ...
      const todayData = await this.getTodayData(botId, null, "single");
